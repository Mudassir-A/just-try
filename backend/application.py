...
# Load Myntra Dataset
myntra = pd.read_csv('data/myntra.csv')

# Sig file which contains similarity between each product
# sig = pickle.load(open('models/sig.pkl', 'rb'))
# Load the .npz file
# Load the compressed numpy array

# Load the compressed numpy array
with gzip.open('data/sig1.npy.gz', 'rb') as f:
    sig1 = np.load(f)

with gzip.open('data/sig2.npy.gz', 'rb') as f:
    sig2 = np.load(f)

with gzip.open('data/sig3.npy.gz', 'rb') as f:
    sig3 = np.load(f)

with gzip.open('data/sig4.npy.gz', 'rb') as f:
    sig4 = np.load(f)
...
embeddings = np.array(pickle.load(open('models/embeddings.pkl', 'rb')))



# all popular products in men's category
# men_popular = pickle.load(open('models/men_popular.pkl', 'rb'))
men_popular = pd.read_pickle(r'models/men_popular.pkl')

# all popular products in women's category
women_popular = pd.read_pickle(r'models/women_popular.pkl')

# all popular products in
popular_products = pd.read_pickle(r'models/popular_products.pkl')


filtered_indices = pd.read_pickle(r'models/filtered_indices.pkl')
...
@application.route('/test-cors', methods=['GET'])
def test_cors():
